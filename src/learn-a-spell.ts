...
const rarityDC: Record<Rarity, number> = {
  common: 0,
  uncommon: 2,
  rare: 5,
  unique: 10,
};
function adjustDCByRarity(dc: number, spell: { rarity: Rarity }): number {
...
const spellcasting: { addSpell: Function, highestRank: number; spells: any, tradition: string } =
  actor.spellcasting.spellcastingFeatures[0];
...
let spell: { name: string; rank: number; uuid: string } | null = null;
...
await Dialog.wait({
  ...
  render: (html: any) => {
    ...
    dragSpellEl.ondrop = async (event: DragEvent) => {
...
let targetDC = adjustDCByRarity(
  learnSpellData.find((d) => d.rank === targetRank).dc,
  spell
);
