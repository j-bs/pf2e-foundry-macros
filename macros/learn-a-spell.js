...
if (!actor.isSpellcaster || !actor.spellcasting?.spellcastingFeatures[0]) {
    ui.notifications.warn(`You must be a spellcaster and have a spellcasting entry to <em>Learn a Spell</em>.`);
    return;
}
...
const hasMagicalShorthand = !!actor.itemTypes.feat.find((f) => f.system.slug === "magical-shorthand");
const hasSpellbookProdigy = !!actor.itemTypes.feat.find((f) => f.system.slug === "spellbook-prodigy");
let spell = null;
await Dialog.wait({
    title: dialogTitle,
    ...
    render: (html) => {
        ...
        dragSpellEl.ondrop = async (event) => {
            ...
            if (!spell) {
                ui.notifications.error(`Something went wrong, could not load spell details`);
                return;
            }
            if (spell.rank > maxSpellRank) {
                ui.notifications.warn(`You cannot learn Rank ${spell.rank} spells yet.`);
                return;
            }
...
console.log(`Player selected to learn ${spell.name}, a Rank ${spell.rank} spell.`);
...
const rollLearnTemplate = `<p>You are attempting to learn <b>${spell.name} (Rank ${targetRank})</b>.</p>
...
<table style="text-align: center" data-tooltip-class="pf2e">
    ...
    : ""}">
        ...
        <tr style="color:red;${hasSpellbookProdigy
...
const traits = ["concentrate", "exploration"];
const options = [
...
const notes = [
    {
        ...
        text: `<b>Critical Success</b> You learn @UUID[${spell.uuid}] and expend only half of the materials worth <b>${targetPrice / 2}gp</b>.`,
    },
    {
        outcome: ["success"],
        selector: traditionSkills[tradition],
    ...
    },
    {
        ...
        selector: traditionSkills[tradition],
        text: `<b>Critical Failure</b> You do not learn @UUID[${spell.uuid}] and expend half of the materials worth <b>${targetPrice / 2}gp</b>.`,
    },
];
...
const rollResult = await game.pf2e.Check.roll(new game.pf2e.CheckModifier(`learn-a-spell`, actor.skills[traditionSkills[tradition]]), {
    type: ["skill-check"],
...
const success = rollResult.options.degreeOfSuccess === 2 ||
    rollResult.options.degreeOfSuccess === 3;
// add spell to prepared spell list
if (!dryRun && success) {
    if (!spellcasting) {
        ui.notifications.error(`Could not find spellcasting entry to automatically add new spell.`);
    }
    try {
        ...
        console.log(ex);
        ui.notifications.error(`Something went wrong while adding new spell to spellcasting entry.`);
    }
}
// determine price based on outcome
let resultPrice = rollResult.options.degreeOfSuccess === 2
    ? targetPrice
...
if (resultPrice > 0) {
    ...
    if (spendGoldCost && !dryRun) {
        if (!(await actor.inventory.removeCoins({ gp: resultPrice }))) {
            ui.notifications.error(`Failed to deduct expended gold from your inventory. Take a loan from a party member?`);
        }
        else {
            ui.notifications.info(`Spent <b>${resultPrice}gp</b> from inventory for <em>Learn a Spell</em>.`);
        }
    }
